...
export default function CardDisplay(props) {
  ...
  const flip = (index) => {
    ...
    if (
      flippedArray.length > 0 &&
      cardState[index].img === flippedArray[0].img &&
      flippedArray[0].id !== cardState[index].id
    ) {
      setTimeout(() => {
        const deadState = [...cardState];
        deadState[index].display = "dead";
        const deadIndex = cardState.findIndex((element) => {
          return element.display === "front";
        });
      ...
      setTimeout(() => {
        ...
        const lastIndex = cardState.findIndex((element) => {
          return element.display === "front";
        });
  ...
  return (
    <div>
      ...
      <div className="time-banner">
        Time Left: &nbsp;{" "}
        <Countdown time={calcTime(num)} setState={setState} num={num} />
      </div>
      <Switch
        leftElement="vertical"
        rightElement="horizontal"
      ...
      {end === false ? (
        ...
        <End num={num} setState={setState} phrase="🥳 Tadaa! 🥳" />
      )}
    </div>
  );
}
